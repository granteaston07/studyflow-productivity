import { X, CheckSquare, Timer, Flame, Zap, TrendingUp, CalendarDays, Sparkles, BookOpen } from 'lucide-react';
...
const WIDGETS: WidgetDef[] = [
  ...
  },
  {
    id: 'countdown',
    name: 'Countdown',
    description: 'Countdown to your next big deadline.',
    size: 'medium',
    preview: (
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-3 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">A preview of what's coming</p>
          </div>
        ...
        {/* Coming soon banner */}
        <div className="mx-5 mb-3 flex-shrink-0 flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-primary/10 border border-primary/25">
          <Sparkles className="h-4 w-4 text-primary flex-shrink-0" />
          <p className="text-xs font-semibold text-primary">Coming Soon — widgets are in development</p>
        </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {group.map(widget => (
                    <div key={widget.id} className="flex flex-col gap-2">
                      <div className={`${SIZE_DIMS[widget.size]} ${size === 'large' ? 'w-full' : ''}`}>
                        {widget.preview}
                      </div>
                      <div>
